...
public:
	...
	C& GetComponent() const {
		C* tmp = GetComponentPtr<C>();
		if (tmp) return *tmp;
		throw InvalidOperationException(__FUNCSIG__, u"保持していないComponentが要求されました。", __FILE__, __LINE__);
	}
	template<CComponent C>
	...
	S& GetScript() const {
		S* tmp = GetScriptPtr<S>();
		if (tmp) return *tmp;
		throw InvalidOperationException(__FUNCSIG__, u"保持していないScriptが要求されました。", __FILE__, __LINE__);
	}
	template<CScript S>
