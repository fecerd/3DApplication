...
export namespace System::Application::Windows::DirectX::Helpers {
	IDXGIFactory4* CreateDXGIFactory() {
		...
		else throw Exception(u"DXGIFactoryの生成に失敗しました。");
	}
	...
	IDXGIAdapter* CreateDXGIAdapter(IDXGIFactory4& factory) {
		...
		if (ret) return ret;
		else throw Exception(u"DXGIAdapterの取得に失敗しました。");
	}
	IDXGISwapChain4* CreateDXGISwapChain(IDXGIFactory4& factory, ID3D12CommandQueue& cmdQueue, HWND hWnd, uint32_t bufferCount) {
		if (!hWnd) throw LogicException(u"DXGISwapChainの生成にはnullptrでないウィンドウハンドルが必要です。");
		...
		else throw Exception(u"DXGISwapChainの生成に失敗しました。");
	}
}
//Direct3D初期化用
export namespace System::Application::Windows::DirectX::Helpers {
	void EnableDebugLayer() {
		...
		if (D3D12GetDebugInterface(IID_PPV_ARGS(&debugLayer)) != S_OK)
			throw Exception(u"デバッグインターフェースの取得に失敗しました。");
	...
	ID3D12Device* CreateDevice(IDXGIAdapter& adapter) {
...
#if defined(__GNUC__)
			//msys2 does not support Feature level 12_2
#else
		...
		else throw Exception(u"D3DDeviceの生成に失敗しました。");
	}
	ID3D12CommandAllocator* CreateCommandAllocator(ID3D12Device& device) {
		ID3D12CommandAllocator* ret = nullptr;
		if (device.CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&ret)) == S_OK) return ret;
		else throw Exception(u"D3DCommandAllocatorの生成に失敗しました。");
	}
	ID3D12GraphicsCommandList* CreateCommandList(ID3D12Device& device, ID3D12CommandAllocator& allocator, ID3D12PipelineState* initialState = nullptr) {
		...
		if (device.CreateCommandList(0, D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_DIRECT, &allocator, initialState, IID_PPV_ARGS(&ret)) == S_OK) return ret;
		else throw Exception(u"D3DCommandListの生成に失敗しました。");
	}
	ID3D12CommandQueue* CreateCommandQueue(ID3D12Device& device) {
		...
		else throw Exception(u"D3DCommandQueueの生成に失敗しました。");
	}
	ID3D12Fence* CreateFence(ID3D12Device& device) {
		ID3D12Fence* ret = nullptr;
		if (device.CreateFence(0, D3D12_FENCE_FLAGS::D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&ret)) == S_OK) return ret;
		else throw Exception(u"D3DFenceの生成に失敗しました。");
	}
}
//GPU同期用
export namespace System::Application::Windows::DirectX::Helpers {
...
export namespace System::Application::Windows::DirectX::Helpers {
	...
	ID3D12Resource* CreateTextureResource(ID3D12Device& device, uint64_t width, uint32_t height) noexcept {
		...
		resDesc.Flags = D3D12_RESOURCE_FLAGS::D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;	//レンダーターゲットに設定できる
		...
		if (device.CreateCommittedResource(&prop, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &resDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, &clear, IID_PPV_ARGS(&tmp)) == S_OK) return tmp;
		else return nullptr;
	}
	ID3D12Resource* CreateEmptyBuffer(ID3D12Device& device, uint64_t dataSize) {
		...
		bufDesc.Dimension = D3D12_RESOURCE_DIMENSION::D3D12_RESOURCE_DIMENSION_BUFFER;
		bufDesc.Width = dataSize;
		bufDesc.Height = 1;
		bufDesc.DepthOrArraySize = 1;
		bufDesc.MipLevels = 1;
		bufDesc.Format = DXGI_FORMAT::DXGI_FORMAT_UNKNOWN;
		bufDesc.SampleDesc.Count = 1;
		bufDesc.Flags = D3D12_RESOURCE_FLAGS::D3D12_RESOURCE_FLAG_NONE;
		bufDesc.Layout = D3D12_TEXTURE_LAYOUT::D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
		ID3D12Resource* ret = nullptr;
		if (device.CreateCommittedResource(&prop, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &bufDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&ret)) < S_OK)
			throw Exception(u"バッファの生成に失敗しました。");
		return ret;
	}
	template<class T>
	ID3D12Resource* CreateBuffer(ID3D12Device& device, const T* data, size_t count) {
		if (!data) throw LogicException(u"nullptrからバッファを作成することはできません。");
		const size_t size = sizeof(T) * count;
		ID3D12Resource* buffer = CreateEmptyBuffer(device, size);
		T* map = nullptr;
		if (buffer->Map(0, nullptr, reinterpret_cast<void**>(&map)) < S_OK) throw Exception(u"バッファのマップに失敗しました。");
		Copy(data, map, size);
...
export namespace System::Application::Windows::DirectX::Helpers {
	...
	ID3D12Resource* CreateDepthStencilResource(ID3D12Device& device, uint64_t width, uint32_t height) noexcept {
		...
		ID3D12Resource* tmp = nullptr;
		if (device.CreateCommittedResource(&prop, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_DEPTH_WRITE, &clear, IID_PPV_ARGS(&tmp)) == S_OK) return tmp;
		else return nullptr;
	}
