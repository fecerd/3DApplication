...
namespace System {
	Type::Type(const Type& arg) noexcept : m_templateCount(arg.m_templateCount) {
		if (arg.m_typeName) m_typeName = new String(*arg.m_typeName);
		if (arg.m_templateArgs) {
			m_templateArgs = new String[m_templateCount];
			for (size_t i = 0; i < m_templateCount; ++i) m_templateArgs[i] = arg.m_templateArgs[i];
		}
	}
	...
	bool Type::operator==(const Type& rhs) const noexcept {
		if (!m_typeName || !rhs.m_typeName) return m_typeName == rhs.m_typeName;
		if (*m_typeName != *rhs.m_typeName) return false;
		if (m_templateCount != rhs.m_templateCount) return false;
		for (size_t i = 0; i < m_templateCount; ++i) {
			if (m_templateArgs[i] != rhs.m_templateArgs[i]) return false;
		}
		return true;
	}
}